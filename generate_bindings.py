...
def generate_interceptor_methods(functions):
    ...
    for func in functions:
        ...
        if name == "socket":
            ...
            result.append("        debug(\"socket(%d, %d, %d) = %d\", domain, type, protocol, sockfd);")
            result.append("    }")
            ...
            result.append("            debug(\"bind(%d, %s:%d) = %d\", sockfd, ")
            result.append("                  inet_ntoa(addr_in->sin_addr), ")
            result.append("                  ntohs(addr_in->sin_port), ret);")
            result.append("        } else {")
            result.append("            debug(\"bind(%d, family=%d) = %d\", sockfd, addr->sa_family, ret);")
            result.append("        }")
            result.append("    }")
            ...
            result.append("            if (addr->sa_family == AF_INET) {")
            result.append("                struct sockaddr_in *addr_in = (struct sockaddr_in *)addr;")
            result.append("                debug(\"connect(%d, %s:%d) = %d\", sockfd, ")
            result.append("                      inet_ntoa(addr_in->sin_addr), ")
            result.append("                      ntohs(addr_in->sin_port), ret);")
            result.append("            } else {")
            result.append("                debug(\"connect(%d, family=%d) = %d\", sockfd, addr->sa_family, ret);")
            result.append("            }")
            result.append("        }")
            result.append("        socklen_t local_addrlen = sizeof(struct sockaddr_storage);")
            result.append("        if (getsockname(sockfd, (struct sockaddr *)&m_socketInfo[sockfd].localAddr, &local_addrlen) != 0) {")
            ...
            result.append("            struct sockaddr_in *addr_in = (struct sockaddr_in *)addr;")
            result.append(f"            debug(\"{name}(%d{flags_param}) = %d from %s:%d\", sockfd, new_sockfd,")
            result.append("                  inet_ntoa(addr_in->sin_addr), ")
            result.append("                  ntohs(addr_in->sin_port));")
            result.append("        } else {")
            result.append(f"            debug(\"{name}(%d{flags_param}) = %d\", sockfd, new_sockfd);")
            result.append("        }")
            result.append("    }")
            ...
            result.append(f"            debug(\"{name}(%d, len=%zu{debug_param}) = %zd\", {fd_param}, {('len' if name == 'send' else 'count')}{debug_param}, ret);")
            result.append("        }")
            result.append("    }")
            result.append("    return ret;")
            ...
            result.append(f"            debug(\"{name}(%d, len=%zu{debug_param}) = %zd\", {fd_param}, {('len' if name == 'recv' else 'count')}{debug_param}, ret);")
            result.append("        }")
            result.append("    }")
            result.append("    return ret;")
        elif name == "sendto":
            ...
            result.append("            debug(\"sendto(%d, len=%zu, %s:%d) = %zd\", sockfd, len,")
            result.append("                  inet_ntoa(addr_in->sin_addr), ")
            result.append("                  ntohs(addr_in->sin_port), ret);")
            result.append("        } else {")
            result.append("            debug(\"sendto(%d, len=%zu) = %zd\", sockfd, len, ret);")
            result.append("        }")
            result.append("    }")
            result.append("    return ret;")
        elif name == "recvfrom":
            ...
            result.append("            debug(\"recvfrom(%d, len=%zu, %s:%d) = %zd\", sockfd, len,")
            result.append("                  inet_ntoa(addr_in->sin_addr), ")
            result.append("                  ntohs(addr_in->sin_port), ret);")
            result.append("        } else {")
            result.append("            debug(\"recvfrom(%d, len=%zu) = %zd\", sockfd, len, ret);")
            result.append("        }")
            result.append("    }")
            result.append("    return ret;")
        elif name == "close":
            ...
            result.append("        debug(\"close(%d) = %d\", fd, ret);")
            result.append("    }")
            result.append("    return ret;")
        else:
