...
def generate_interceptor_methods(functions):
    ...
    for func in functions:
        ...
        if name == "socket":
            ...
            result.append("        log(\"socket(%d, %d, %d) = %d\\n\", domain, type, protocol, sockfd);")
            result.append("    }")
            ...
            result.append("            if (addr->sa_family == AF_INET) {")
            result.append("                struct sockaddr_in *addr_in = (struct sockaddr_in *)addr;")
            result.append("                log(\"connect(%d, %s:%d) = %d\\n\", sockfd, ")
            result.append("                      inet_ntoa(addr_in->sin_addr), ")
            result.append("                      ntohs(addr_in->sin_port), ret);")
            result.append("            } else {")
            result.append("                log(\"connect(%d, family=%d) = %d\\n\", sockfd, addr->sa_family, ret);")
            result.append("            }")
            result.append("        }")
            result.append("        socklen_t local_addrlen = sizeof(struct sockaddr_storage);")
            result.append("        if (getsockname(sockfd, (struct sockaddr *)&m_socketInfo[sockfd].localAddr, &local_addrlen) != 0) {")
            ...
            result.append("            struct sockaddr_in *addr_in = (struct sockaddr_in *)addr;")
            result.append(f"            log(\"{name}(%d{flags_param}) = %d from %s:%d\\n\", sockfd, new_sockfd,")
            result.append("                  inet_ntoa(addr_in->sin_addr), ")
            result.append("                  ntohs(addr_in->sin_port));")
            result.append("        } else {")
            result.append(f"            log(\"{name}(%d{flags_param}) = %d\\n\", sockfd, new_sockfd);")
            result.append("        }")
            result.append("    }")
            ...
            result.append("            log(\"sendto(%d, len=%zu, %s:%d) = %zd\\n\", sockfd, len,")
            result.append("                  inet_ntoa(addr_in->sin_addr), ")
            result.append("                  ntohs(addr_in->sin_port), ret);")
            result.append("        } else {")
            result.append("            log(\"sendto(%d, len=%zu) = %zd\\n\", sockfd, len, ret);")
            result.append("        }")
            result.append("    }")
            result.append("    return ret;")
        elif name == "recvfrom":
            ...
            result.append("            log(\"recvfrom(%d, len=%zu, %s:%d) = %zd\\n\", sockfd, len,")
            result.append("                  inet_ntoa(addr_in->sin_addr), ")
            result.append("                  ntohs(addr_in->sin_port), ret);")
            result.append("        } else {")
            result.append("            log(\"recvfrom(%d, len=%zu) = %zd\\n\", sockfd, len, ret);")
            result.append("        }")
            result.append("    }")
            result.append("    return ret;")
        elif name == "close":
